...
const commands = {
  ...
  sync: () => {
    ...
    const settingsFile = path.join(projectConfigDir, 'settings.json');

    // Create .claude directory
    ensureDir(projectConfigDir);

    // Auto-migrate old settings format
    if (fs.existsSync(settingsFile)) {
      try {
        ...
        if (isOldFormat) {
          console.log('');
          console.log('⚠️  Detected old hooks format, migrating...');
        ...
        // Invalid JSON, will be handled later
      }
    }
